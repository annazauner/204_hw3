...
author: 'Anna Zauner, Kirsten White and Katherine Rosencrance'
date: "May 11, 2022"
...
cs_high_tax <- (0.5 * (.3161 - price_kWh - (p_tax/100)) * demand_high_tax) - (demand_high_tax * price_kWh)
...
cs_low_tax <- (0.5 * (.2337 - price_kWh - (p_tax/100)) * demand_low_tax) - (demand_low_tax * price_kWh) - env_cost_tax
...
# PULL FROM PREVIOUS COST WITH TAX VALUE
...
Decreases to: **$`r round(env_cost_tax, 0)`**
...
demand_scc <- function(p, model, scc){
  q <- (p - (model$coefficients[[1]] - tax_list[scc]))/model$coefficients[[2]]
  q <- ifelse(q<0,0,q)
  return(q)
} 
...
welfare_low_75 <- cs_low_75 - (demand_low_75 * .0289) - revenue_damage_75 + low_use_percentage*revenue_damage_75
...
welfare_high_75 <- cs_high_75 - (demand_high_75*0.0289) + high_use_percentage*revenue_damage_75
# Producer @ $75
