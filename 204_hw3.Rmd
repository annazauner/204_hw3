...
```{r}
# MOVE Q TO ONE SIDE TO HORIZONTALLY SUM
...
demand_agg_p <- function(price_cents){kwh_agg_slope*(price_cents)+ kwh_agg_int}
...
- Environmental Cost = $`r round(env_cost_base, 0)`
...
As calculated above, total consumer surplus is $53,518.
This value is derived from an aggregate demand curve where 522,128 kWh will be consumed at price of $0.10/kWh. 

To find the surplus differential between income groups, we use the current price and calculate the amount each group will consume at this level. 

```{r}

### We didn't create these equations eariler, so this code won't run 
